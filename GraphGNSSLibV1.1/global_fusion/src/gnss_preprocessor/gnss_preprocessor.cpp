...
//    std::cout << "EmpFile: " << EmpFile << std::endl;
//    std::cout << "out_folder: " << out_folder << std::endl;
    ...
    double ts_time, te_time;
    ros::param::get("ts_time", ts_time);
    ros::param::get("te_time", te_time);
    gtime_t ts={0},te={0};
    ts.time=ts_time;							// start time (0:all)
    te.time=te_time;							// end time (0:all)
