...
static int ddmat(rtk_t *rtk, double *D)
{
    ...
    for (i=0;i<MAXSAT;i++) for (j=0;j<NFREQ;j++) { //MAXSAT:90
        rtk->ssat[i].fix[j]=0;
    }
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
    ...
    // Q_matrix.resize(n,n);
    Q_matrix = Eigen::Map<Matrix<double, Dynamic, Dynamic,ColMajor>>(tt,nb,nb);
    ...
    if (nb > 4) {
        for (int i=0; i<nb; i++) {
            for (int j=0; j<nb; j++) {
                Q_matrix(i,j) = Qb[i*nb+j];
            }
        }
        double ADOP = pow(Q_matrix.determinant(), 1.0/(2 * float(nb)));
    }
...
//    for(int i=0; i<n; i++) {
//        LOG(INFO)<<"sat_pos "<< i <<" -> id: " << float(obs[i].sat) << " " << rs[0 + i * 6] << " " << rs[1 + i * 6] << " " << rs[2 + i * 6];
//    }
...
//            LOG(INFO)<<"1788";
            ...
            gnss_raw.LLI = obs[ir[is]].LLI[0];
             //simple detection based on LLI, we can add other logic to detect CS
            gnss_raw.slip = (obs[ir[is]].LLI[0] & 3) > 0 ? 1 : 0;
//            LOG(INFO)<<"1795";
...
//            LOG(INFO)<<"rs_-> "<<rs_[0] << " " << rs_[1] << " " << rs_[2];
//            LOG(INFO)<<"rr_-> "<<rr_[0] << " " << rr_[1] << " " << rr_[2];

//            LOG(INFO)<<"e_-> "<<e_[0] << " " << e_[1] << " " << e_[2];
//            LOG(INFO)<<"pos_-> "<<pos_[0] << " " << pos_[1] << " " << pos_[2];
//
//            LOG(INFO)<<"azel_->--------------------- "<<azel_[0] * R2D;
//            LOG(INFO)<<"azel_->-------------------- "<<azel_[1] * R2D;
//            LOG(INFO)<<"1812";
            ...
            for (int j=0; j<NFREQ+NEXOBS;j++) {
                gnss_raw_mf.lamda.push_back(nav->lam[obs[ir[is]].sat][j]);
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    */
    ...
    if(current_tow<0) sleepms(1); //start_gps_sec
    else 
        sleepms(delayms);

    double time_d = obs[0].time.time + obs[0].time.sec;
    ...
    if(current_tow<0) sleepms(2); //start_gps_sec
    else 
        sleepms(delayms);
