...
static double varerr(const prcopt_t *opt, double el, int sys)
{
    ...
    // std::cout<<"varr-> " << SQR(fact)*varr <<std::endl;
    // std::cout<<"opt->err[0]-> " << opt->err[0] <<std::endl;
    // std::cout<<"opt->err[1]-> " << opt->err[1] <<std::endl;
    // std::cout<<"opt->err[2]-> " << opt->err[2] <<std::endl;
                  ...
                  char *msg)
{
    ...
    for(int s_i=0;s_i<n; s_i++)
    {
        ...
        gnss_raw_mf.prn_satellites_index = float(obs[s_i].sat);
        ...
        gnss_raw_mf.azimuth = azel_[0 + s_i*2] * R2D;
        gnss_raw_mf.elevation = azel_[1 + s_i * 2] * R2D;
...
//            gnss_raw_mf.raw_carrier_phase.push_back(obs[s_i].L[j]);
    ...
    if(fabs(current_tow-lastGNSSTime)>0.0) // 0.4
    {
...
//    pub_gnss_raw.publish(gnss_data);
    ...
    if (ssat) {
        ...
        for (i=0;i<n;i++) {
            ...
            ssat[obs[i].sat-1].prn = double(obs[i].sat); // dk add
        }
    }
