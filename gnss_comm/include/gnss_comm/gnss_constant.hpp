...
static const char rcsid[]="$Id: lamda.c,v 1.1 2008/07/17 21:48:06 ttaka Exp $";
...
namespace gnss_comm
{
    ...
    #define EPH_VALID_SECONDS       7200                // 2 hours ephemeris validity 7200
    ...
    /* single satellite observation (e.g. master satellite for DD) */
    struct singleObs    
    {
        gtime_t time; 
        double prn_satellites_index;
        double pseudorange;
        ...
        double sat_pos_y;
        double sat_pos_z;
        double ttx;//# satellite obstime - tof - satellite clock bias
        double vel_x;// satellite velocity 
        double vel_y;// satellite velocity 
        double vel_z;// satellite velocity 
        double dt; // satellite clock drift
        double ddt; // satellite clock drift rate
        double tgd; /* group delay parameters (Prange function in RTKLIB) */
    ...
    struct Obs                                           /* observation data record */
    {
        ...
        /* User gnss msg ： user to ith satellite */
        double Uprn_satellites_index; // satellite id
        double Upseudorange;  // satellite pseudorange (free of atmos and sat clock error)
        double Uraw_pseudorange; // raw pseudorange
        double Ucarrier_phase; // carrier-phase measurements
        double Ulamda; // wave length
        double Usnr; // signal noise ratio
        double Uelevation; // satellite elevation
        double Uazimuth; // satellite azimuth
        double Uerr_tropo; // tropo error
        double Uerr_iono; // iono error
        double Usat_clk_err; // satellite clock error
        double Usat_pos_x;
        double Usat_pos_y;
        double Usat_pos_z;
        double Uttx;//# obstime - tof - satellite clock bias
        double Uvel_x;// satellite velocity 
        double Uvel_y;// satellite velocity 
        double Uvel_z;// satellite velocity  
        double Udt; // satellite clock drift
        double Uddt; // satellite clock drift rate
        double Utgd; /* group delay parameters */
        int Uvisable;// #0-Not sure 1-visable 2-invisable
        std::string Usat_system;
        int Uvisable3DMA;// # ground truth visibility from 3DMA
        double UprE3dMA;//# ground truth pseudorange error from 3DMA
        ...
        */
        singleObs u_m_sat; // u_master_sv: user to master 
        singleObs r_m_sat; // r_master_sv: reference to master satellite
        singleObs r_i_sat; // r_i_sv: reference to ith satellite

        bool DD_psr; // satisfy the DD pseudorange measurements or not
        bool DD_car; // satisfy the DD carrier-phase measurements or not
        double IR; // integer ambiguity
    };
